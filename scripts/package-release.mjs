...
const roots = ["src", "scripts", "public", ".github"];
...
const localSecrets = existsSync(resolve(root, ".env.local"))
  ? readFileSync(resolve(root, ".env.local"), "utf8").split(/\r?\n/).filter((line) => /^(SESSION_SECRET|WORKSPACE_ENCRYPTION_KEY)=/.test(line)).map((line) => line.slice(line.indexOf("=") + 1).trim().replace(/^["']|["']$/g, "")).filter((value) => value.length >= 32)
  : [];
const entries = {};
